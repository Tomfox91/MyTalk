...
(function () {
            ...
            getStats: function(callback) {
                peerConnection.getStats(function(stats) {
                    ...
                    for (var i = 0; i < results.length; ++i) {
                        var res = results[i];
                        tbr.timestamp = res.timestamp;
                        if (res.stat('bytesSent'))
                            tbr.bytesSent += parseInt(res.stat('bytesSent'));
                    }
                    callback(tbr);
                });
            }
    ...
    function RTCMultiSession(config) {
            ...
            function socketResponse(response) {
                ...
                if (response.left) {
                    ...
                    if (response.closeEntireSession) {
                        ...
                        for (var j = 0; j < self.oldIds.length; j++) {
                            socket.send({
                                left: true,
                                extra: self.extra,
                                id: self.oldIds[j]
        ...
        function leaveARoom(channel) {
            ...
            if (isbroadcaster) {
                if (config.autoCloseEntireSession) alert.closeEntireSession = true;
                    ...
                    var i = 0;
                    while (i < self.sockets.length && !self.sockets[i]) i++;
                    if (self.sockets[i])
                        self.sockets[i].send({
            ...
            if (!channel) {
                ...
                for (var i = 0; i < length; i++) {
                    ...
                    if (socket) {
                        for (var j = 0; j < self.oldIds.length; j++) {
                            socket.send(mkAlert(self.oldIds[j]));
                        }
        ...
        (function () {
            ...
            for (var i = 0; i < length; i++) {
                var a = anchors[i];
                if (a.getAttribute('href').indexOf('#') !== 0 && a.getAttribute('target') != '_blank') a.onclick = function () {
                    leaveARoom();
                };
            }
        })();
        ...
        return {
            initSession: function (extra) {
                ...
                extra.interval = extra.interval || 1000;
